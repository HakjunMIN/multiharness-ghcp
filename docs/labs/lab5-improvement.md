...
- core 동작과 UI가 같은 HTTP contract를 사용하게 한다.
...
core는 no-evidence behavior를 vertical ticket으로 끝낸다. full은 이 core 동작을
먼저 끝낸 뒤 React chat, loading/error/429 상태, structured citation rendering을
추가한다.
...
- core behavior ticket 하나가 red-green 근거와 함께 commit됐다.
