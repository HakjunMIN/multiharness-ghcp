...
production code는 고치지 마세요. 이 ticket은 실패하는 브라우저 시나리오만
남기고 끝나며, 통과시키려고 단언을 약하게 고치지 마세요.
```

`<feature>`와 ticket 파일 이름을 실제 경로로 바꿉니다. `implement` 스킬의
기본 동작은 구현해서 green을 만드는 것이므로, 구현하지 않고 실패를 남긴다는
이 랩의 반대 조건만 프롬프트로 명시합니다. 나머지 범위와 acceptance criteria는
ticket 문서에 있습니다.
