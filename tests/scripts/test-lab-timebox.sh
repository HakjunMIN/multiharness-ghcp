...
for day in 1 2; do
  minutes="$(sed -n "s/^| ${day}일차 합계 | \([0-9][0-9]*\)분 |/\1/p" docs/instructor/timebox.md)"
  if [ "$minutes" != "330" ]; then
    echo "day ${day} total is ${minutes:-missing}, expected 330" >&2
    exit 1
  fi
done
...
for lab in docs/labs/lab0-*.md docs/labs/lab1-*.md docs/labs/lab2-*.md \
  docs/labs/lab3-*.md docs/labs/lab4-*.md docs/labs/lab5-*.md docs/labs/lab6-*.md \
  docs/labs/lab7-*.md docs/labs/lab8-*.md; do
