...
make_fixture() {
  ...
  cp "$ROOT/scripts/check-matt-skills.mjs" "$target/scripts/"
  cp "$ROOT/scripts/required-matt-skills.txt" "$target/scripts/"
  node - "$target" "$ROOT/scripts/required-matt-skills.txt" <<'NODE'
...
cp "$ROOT/scripts/check-matt-skills.mjs" "$TMP/empty/scripts/"
cp "$ROOT/scripts/required-matt-skills.txt" "$TMP/empty/scripts/"
check_ok "pre-install state is optional" \
  bash -c "cd '$TMP/empty' && node scripts/check-matt-skills.mjs"
check_fail "required mode rejects a missing lock" \
  bash -c "cd '$TMP/empty' && node scripts/check-matt-skills.mjs --required"

make_fixture "$TMP/valid"
check_ok "complete installation passes" \
  bash -c "cd '$TMP/valid' && node scripts/check-matt-skills.mjs --required"

cp -R "$TMP/valid" "$TMP/missing-file"
rm -rf "$TMP/missing-file/.agents/skills/tdd"
check_fail "missing installed directory fails" \
  bash -c "cd '$TMP/missing-file' && node scripts/check-matt-skills.mjs --required"
...
check_fail "missing lock entry fails" \
  bash -c "cd '$TMP/missing-lock' && node scripts/check-matt-skills.mjs --required"
...
check_fail "wrong source fails" \
  bash -c "cd '$TMP/wrong-source' && node scripts/check-matt-skills.mjs --required"
...
check_fail "invalid hash fails" \
  bash -c "cd '$TMP/bad-hash' && node scripts/check-matt-skills.mjs --required"
...
printf 'OK: Matt skill contract tests passed\n'
