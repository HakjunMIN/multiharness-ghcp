...
| 용어 | 뜻 |
| --- | --- |
| 근거 후보 | 검색 결과 중 질문에 답하는 내용을 포함하지만 아직 출처 적격성을 확인하지 않은 자료. |
| 출처 등급 | 발행 주체와 편집 책임에 따라 근거 후보의 신뢰 수준을 1~3등급으로 분류한 값. |
| 적격 근거 | 출처 등급별 채택 조건을 충족해 답변과 citation에 사용할 수 있는 근거. |
| 독립 출처 | 작성자나 원문이 서로 다르고 다른 게시물을 단순 전재하지 않은 출처. |
| 근거 없음 | 검색 결과가 없는 상태뿐 아니라 모든 근거 후보가 출처 등급별 채택 조건을 충족하지 못한 상태. |
| 근거 상충 | 둘 이상의 적격 근거가 같은 사실에 대해 양립할 수 없는 내용을 제시하는 상태. |
